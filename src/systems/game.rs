...
pub fn game_tick_system(mut game:ResMut<Game>, game_pieces:Query<(Entity, &GamePiece)>, mut commands: Commands, mut new_game_reader:EventReader<NewGameEvent>, time:Res<Time>, mut hud:ResMut<Hud>, mut game_state_change_writer:EventWriter<GameStateChangeEvent>) {
    tick(&mut game, &mut hud, &time, &mut game_state_change_writer);
