...
pub fn tank_system(tank:Query<(&mut Tank, &RigidBodyHandleComponent)>, mut rigid_body_set:ResMut<RigidBodySet>, time:Res<Time>, sprites:Query<&mut TextureAtlasSprite>) {
    tank.for_each_mut(|(mut tank, rigid_body)| {
        if let Some(rigid_body) = rigid_body_set.get_mut(rigid_body.handle()) {
            ...
            if tank.tracks_distance.length() > 0.1 {
                println!("test");
